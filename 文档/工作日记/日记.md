# TODO #
	...
	(on going)21. 转账交易是否需要收手续费? (测试方法:一个账号中只有1个wei,然后向另外一个账号转一个wei,看是否能够转成功.)
		case1:
			节点启动: gwan --datadir "./" --networkid 168 --etherbase "0xa3dae027322ae417b4880bd418bf005eddd77b3a" --unlock "0xa3dae027322ae417b4880bd418bf005eddd77b3a" console
			账户中只有1个win,然后发送转账交易: eth.sendTransaction({from:eth.accounts[1],to:eth.accounts[0],gasPrice:0,value:web3.toWin(1,'win')})
			结果: 转账成功,账户余额为0,交易被打包到了块中. 说明这种情况下,是能够发送gasPrice为0的交易的.
			问题:1. 如果有大量的gasprice不为0的交易,那么这个gasPrice为0的交易是不是就不能够被及时执行了?
				 2. 如果节点设置了gasprice为1的话那么这个交易是不是就不能被包含在块中了? (答:仍然能.)
					节点: gwan --datadir "./" --networkid 168 --etherbase "0xa3dae027322ae417b4880bd418bf005eddd77b3a" --unlock "0xa3dae027322ae417b4880bd418bf005eddd77b3a" --gasprice 1 console
					交易: eth.sendTransaction({from:eth.accounts[1],to:eth.accounts[0],value:web3.toWin(1,'win')}) 不设置交易的gasprice为0.
					结果: 交易仍然能够被打包到块中(如果设置了交易的gasprice为0的话结果不变,交易仍然能被打包到块中).
